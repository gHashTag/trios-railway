...
**LAST UPDATE:** 2026-05-01 — Based on actual execution findings.
...
| Workers stopping at step 1000 | ⚠️ Under investigation | Possible checkpoint/write issue |
| Exit status 101 crashes | ⚠️ Under investigation | Likely image/GLIBC mismatch |
| Writer flowing slowly | ✅ Working | 2 rows/5min (acceptable with 4 dead accounts) |
| Timeout failures | ❌ FALSE hypothesis | No timeout in last 2h — not the issue |

**Resolution required:** Manual Railway dashboard intervention OR user-level `RAILWAY_TOKEN`.
